// Given a weighted tree and two nodes X and Y, find the length of the path between X and Y.
#include<bits/stdc++.h>
#define pb push_back
...
const int M = 22;
int parent[N], dep[N], tin[N], tout[N], timer, dis[N];
int Par[N][M]; // Par[i][j] = 2^j th parent of i

void dfs(int node, int par, vector<vector<pii>> &graph){ // O(n)
    tin[node] = ++timer;
    parent[node] = par;
...
void cal_sparse_table(int node, int parent, vector<vector<pii>> &graph){ // O(n)*M
    Par[node][0] = parent;
    for(int i=1;i<M;i++){
    ...
    for(auto child: graph[node]){
        if(child.first != parent){
            cal_sparse_table(child.first, node, graph);
        }
    }
}
