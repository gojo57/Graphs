...
void dfs(int node, vvi &adj, vi &vis, int &chk, int &o, int &e, int parity = 0){
    if(vis[node]) {
        if(1+parity != vis[node]){
            chk = 0;
        }
        return;
    }
    ...
    for(auto it: adj[node]){
        dfs(it, adj, vis, chk, o, e, 1-parity);
    }
}
...
signed main()
{
    ...
    while(test--){
        ...
        for(int i = 1; i<=n; i++){
            if(!vis[i]){
                int chk = 1, o = 0, e = 0;
                dfs(i, adj, vis, chk, o, e, 0);
                if(!chk){
                    ans = 0;
                    break;
                }
                ans = (ans*(pwmd(2, e)+pwmd(2, o)))%mod;
            }
        }     
        cout<<ans<<"\n";
    }
