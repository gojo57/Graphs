...
vector<string> bfs(map<string, int> &mp, map<string, vector<string>> &adj){
    vector<string> ans;
    queue<string> q;
    for(auto it: mp){
        if(it.second == 0){
...
signed main()
{
    ...
    while(cin>>n){
        ...
        vector<string> ans = bfs(mp, adj);
        cout<<"Case #"<<count<<": Vivek should drink beverages in this order: ";
